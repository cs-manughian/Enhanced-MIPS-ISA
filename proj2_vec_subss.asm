...
#
# File name:        vec_subss.asm
# Version:          1.0
# Date:             April 13, 2015  
# Programmer:       Nancy Torres
#
# Description:      This enhanced instruction performs a vector subtraction saturated signed. 
#		    Each element of vector b is subtracted from the corresponding element of 
#		    vector a. The signed integer (no-wrap) is placed into the corresponding 
#  		    element of vector d. The format is as follows:
#				vec_subss d, a, b
...
#		    $t4: 1. Shift amount for 2's complement 
#		 	 2. Boolean that is false if we need to saturate to highest value
...
main:      #Initializations
	   	...
	   	#Set by user
           	addi $t0, $zero, 0x8067013D	#Vector a		
     	   	...
     	   	addi $t2, $zero, 0xFFC54D23	#Vector b		
     	   	addi $t3, $zero, 0x127701CD			
...
init2:	   	and $v0, $t1, $t7		#Get element
	   	...
	   	subi $s2, $s2, 32		#Reset to point to point to memory location 
	   					#with label "vectora"
	   ...
	   #(Each byte is an element)
	   	...
	   	addi $t7, $zero, 0xFF000000	#Reinitialize to access leftmost element
	   					#in register $t2
     	  	add $t9, $zero, $zero		#Reinitialize shift amount
     	  	addi $t4, $zero, 24		#Initialize shift amount for 2's complement
     	  	#Move $t2 into vector b array		   	
init3:	   	and $v0, $t2, $t7		#Get element
		srlv $v0, $v0, $t4		#Shift to perform 2's complement
		not $v0, $v0			#Invert
		addi $v0, $v0, 1		#Add 1
		andi $v0, $v0, 0x000000FF	#Mask other bits (only want a byte)
		sllv $v0, $v0, $t4		#Shift back to original place
	   	sllv $v0, $v0, $t9		#Shift element all the way to the left
	   					#to make calculations easier
	   	...
	   	addi $t9, $t9, 8 		#Increment shift amount by 8
	   	addi $t4, $t4, -8		#Decrement 2's complement shift amount
...
init4:	   	and $v0, $t3, $t7		#Get element
		...
		sllv $v0, $v0, $t4		#Shift back to original place
	   	...
	   	subi $s3, $s3, 32		#Reset to point to point to memory location 
	   					#with label "vectorb"
...
loop:	   	lw $s6, 0($s2)			#Load vector a array element into $s5
		...
		sub $s2, $s2, 32		#Reset to point to memory location 
	   					#with label "vectora"
		sub $s3, $s3, 32		#Reset to point to memory location 
	   					#with label "vectorb"
		sub $s4, $s4, 32		#Reset to point to memory location 
