...
#
# File name:    	proj2_unitvectorfp.asm
# Version:      	1.0
# Date:         	April 10, 2015  
...
contDiv:
   		 lwc1  $f2, 0($t1)   	     	 #Load vectora[i] into $f2
   		 cvt.s.w $f2, $f2		 #Convert from word to single precision
   		 div.s $f2, $f2, $f4   	     	 #Divide the element by the magnitude and it's stored in $f2
   		  	 
   		 swc1 $f2, 0($t4)   		 #Put the result ($t2) into vector d   	 
