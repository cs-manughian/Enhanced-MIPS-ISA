...
#
# Description:      This baseline instruction performs a vector merge high.
#		The even elements of the result vector d are obtained left-
#		to-right from the high elements of vector a. The odd elements
#		of the result are obtained left-to-right from the high elements
...
main:      #Initializations
	   ...
	   #Put each high element (byte) of vector a into a register
	   	
	   	and $t5, $t0, $t4	#Get byte 4 and put into $t5
	   				#Shifted into leftmost position
	   				
	   	srl $t4, $t4, 8		#Next byte
	   	and $t6, $t0, $t4	#Get byte 5 and put into $t6
	   	srl $t6, $t6, 8		#Shift into byte 2 position
	   	
	   	srl $t4, $t4, 8		#Next byte
	   	and $t7, $t0, $t4	#Get byte 6 and put into $t7
	   	sll $t7, $t7, 16	#Shift into byte 4 position
	   		   	
	   	srl $t4, $t4, 8		#Next byte
	   	and $t8, $t0, $t4	#Get byte 7 and put into $t8	
	   ...
	   #Put each high element (byte) of vector b into a register
	   
	   	sll $t4, $t4, 24	#Reset byte selection to leftmost byte	
	   	and $t5, $t2, $t4	#Get byte 4 and put into $t5
	   	srl $t5, $t5, 8		#Shift into byte 1 position
	   				
	   	srl $t4, $t4, 8		#Next byte
	   	and $t6, $t2, $t4	#Get byte 5 and put into $t6
	   	srl $t6, $t6, 16	#Shift into byte 3 position
	   	
	   	srl $t4, $t4, 8		#Next byte
	   	and $t7, $t2, $t4	#Get byte 6 and put into $t7
	   	sll $t7, $t7, 8		#Shift into byte 5 position
	   		   	
	   	srl $t4, $t4, 8		#Next byte
	   	and $t8, $t2, $t4	#Get byte 7 and put into $t8	
					#In byte 7 position
